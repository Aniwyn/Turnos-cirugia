...
const ViewCashBoxDetail = ({ boxId }) => {
    ...
    useEffect(() => {
        const init = async () => {
            const fetchedBox = await getCashBoxById(boxId)
            setBox(fetchedBox)
            console.log("ASDQWE", fetchedBox)
        }
                            ...
                            <ModalBody>
                                {box &&
                                    <div className="flex flex-col">
                                        ...
                                        {Array.isArray(box.cashMovement) && box.cashMovement.length > 0 && (
                                                ...
                                                {box.cashMovement.map((movement) => (
                                                    <>
                                                        ...
                                                        <div className="col-span-2 flex justify-center">
                                                            {`${formatDateFromDB(movement.created_at)} a las ${formatTimeFromDB(movement.created_at)}`}
                                                        </div>
                                                        <div className="col-span-2 flex justify-end">
                                                            {`${movement.currency == 'ARS' ? '$' : 'USD'} ${movement.type == 'expense' ? '-' : ''} ${formatCurrency(movement.amount)}`}
                                                        </div>
                                                    </>
                                                ))}
                                            </div>
