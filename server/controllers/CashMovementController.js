...
exports.getUserCashMovements = async (req, res) => {
    ...
    try {
        ...
        const movements = await db.CashMovement.findAll({
            ...
            where: { 
                user_id: userId,
                closed: 0 // 0 indica movimientos que no fueron cerrados en otra caja
            },
            include: [
                { association: 'label' }
            ]
        ...
        res.status(500).json({ message: 'Error al obtener movimientos de caja del usuario', error: err })
    }
}
...
exports.createCashMovement = async (req, res) => {
    ...
    try {
        ...
        const response = await db.CashMovement.create( {...movement, user_id: userId })

        return res.status(201).json({ message: "Cash movement created", response: response })
    } catch (err) {
        console.error(err)
        return res.status(500).json({ message: "Error creating cash movement", error: err })
    }
}
